...
ADR-038 accepted limitation. mayu-plane team and provider/model writes
carry the same evidence inside the hash-chained audit record
(`audit.MutationRef`: durable actor, capability, before/after sha256 of
...
`pii_protected_detected`). Detector evidence now rides the audit
chain (2026-09-02): masked requests and `external-unmodified` refusals
carry `pii: {redactions, kinds}` — counts and filter-declared kind names
only, never a matched value, so the chain stays PII-free. Remaining from
the Phase 2 contract: the OpenAI ingress verifies nothing in v1 (no
maskBody there — it refuses `external-unmodified` outright, same posture
