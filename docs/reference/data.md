...
| Store 인터페이스 | `internal/keystore/keystore.go` | `Store`, `Principal`, `Allows()` (RBAC); `TeamStore`(`UpsertTeam`/`GetTeam`/`ListTeams`/`DeleteTeam`, D3)는 별도 인터페이스로 분리되어 `internal/server` 테스트의 기존 `Store` fake들에 영향 없음; `KeyEnsurer`(`EnsureKey`, ADR-023)도 같은 패턴 — 호출자가 준 plaintext를 upsert(`ON CONFLICT(key_hash) DO UPDATE`, `revoked`/`created_at`는 의도적으로 제외)해 config가 선언한 가상 키(`config.VirtualKeys`, `newGateway`에서 `keystore.OpenSQLite` 직후 부트스트랩)가 스토어 재생성 후에도 살아남게 함 — `Create`/`CreateWithOptions`는 항상 새 랜덤 plaintext를 생성한다는 점과 다름 |
