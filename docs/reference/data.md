...
| Provider store | `internal/providerstore/sqlite.go` | opt-in DB topology (ADR-008): `providers` (refs only — no secret column; `guardrail_id`/`guardrail_version` TEXT columns, D6/ADR-019 — a DB-registered Bedrock provider's own default Guardrail, ALTER-TABLE migrations mirroring `auth_header`), `model_targets` (ordered routes; `price_input_per_mtok`/`price_output_per_mtok` REAL + `price_free` INTEGER, ALTER-migrated — a target's optional nominal per-MTok rate, ADR-041 item 6; all-zero columns read back as nil pricing, which is lossless because the write DTO rejects a present-but-all-zero pricing object), `model_aliases` (`model`, `alias` PK — ADR-021 follow-up: a model's alias→canonical names, group-level so a multi-target fallback chain can't duplicate them; a brand-new `CREATE TABLE IF NOT EXISTS`, no ALTER-TABLE needed), `meta` (durable `seeded` marker); Postgres-portable TEXT-only DDL |
...
- Admin-plane MUTATION events (`admin_team_upserted`/`admin_team_deleted`, `provider_registered`/`provider_deleted`, `model_route_updated`/`model_route_deleted`) additionally carry `mutation` (Phase 0b-4, `audit.MutationRef`, appended at the Record END per the same rule): the actor's durable `issuer#sub` (empty for break-glass — never fabricated), the capability the route class required, and before/after sha256 of the canonical STORED row — computed from what the store holds (read back after the write), never from the request body, so the digest attests resulting state; an empty digest is an absence (before on create, after on delete), never the hash of the empty string. Digest inputs are secret-free by construction (team records and provider rows hold refs only). Provider/model digests flow through the OPTIONAL `configapi.Reader` seam the gateway implements over the provider store; a Reader-less Writer still emits the record, just without digests.
