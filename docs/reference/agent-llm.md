...
- The typed detector result (`filter.Detection{Redactions}`, strategy Phase 2) is derived from the SAME `Mask` pass enforcement uses, run detect-only with the output discarded — so an `external-unmodified` egress ceiling's "nothing protected" verdict can never disagree with what masking would have transformed. A verified-clean request egresses on the ORIGINAL raw body (the cache invariant is preserved — nothing was re-serialized); no configured detector, a detector error, or a hit refuses 403 (`pii_detector_unavailable` / `pii_protected_detected`).
