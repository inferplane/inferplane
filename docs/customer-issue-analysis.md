...
**구현 완료(ADR-021):** 이 갭을 실제로 닫았다. (1) config `models.<name>.aliases`로 모델
별칭을 선언하면 `apac.anthropic.*` 같은 이름이 RBAC/라우팅/감사 이전에 canonical로 정규화된다.
(2) 미등록 모델 404·미허용 모델 403 응답이 이제 해당 키가 쓸 수 있는 모델 목록을 함께 반환해
LiteLLM의 `Invalid model name`류 막다른 에러를 대체한다. (3) 2026-07-12 후속으로
`provider_store`(admin UI 등록 모델) 경로에도 동일한 alias 기능을 확장했다 — config 파일
전용이던 제약이 없어져, 콘솔로 등록한 모델도 별칭을 가질 수 있다.
...
**구현 완료(ADR-021):** 개발자 본인이 대시보드/관리자 없이 직접 확인할 수 있도록
`GET /v1/usage`(데이터 플레인, 가상 키 인증)를 추가했다. 호출자 본인의 키별·팀 예산/쿼터를
정수 microUSD로 반환하며(무제한 차원은 null), 응답에 `key_id`를 절대 담지 않는다 —
"한 것도 없는데 한도를 다 썼다"(row 37) 불안의 직접 해소책이다.
