...
- **Admin plane (`:9090`)** -- `/healthz`, `/readyz`, unauthenticated `/metrics`, the token-authenticated `/admin/keys` API, and the minimal embedded key console at `/admin/ui/` (data-free static assets, ADR-001).
...
- **관리 플레인 (`:9090`)** -- `/healthz`, `/readyz`, 무인증 `/metrics`, 토큰 인증 `/admin/keys` API, 그리고 `/admin/ui/`의 최소 내장 키 콘솔(데이터 없는 정적 자산, ADR-001).
...
- 관리 플레인: `:9090` (`/healthz`, `/readyz`, `/metrics`, `/admin/keys`, `/admin/ui/`)
