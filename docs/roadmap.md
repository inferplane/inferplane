...
|---|---|---|
| #1 A single entry point for Claude Code/OpenCode/Codex | 🔶 partial | REAL-client verification (2026-09-02, [docs/verification/coding-agents.md](verification/coding-agents.md)): Claude Code 2.1.258 and OpenCode 1.18.26 each completed a real turn through a running mayu — served, settled, hash-chain-audited, zero client patches. Codex 0.152.1 is BLOCKED: current Codex removed `wire_api: "chat"` for custom providers and requires the OpenAI Responses API (`POST /v1/responses`), which mayu does not serve — the fixture tests (`agent_wire_test.go`) pin a wire current Codex no longer speaks. Closing #1 = a `/v1/responses` ingress |
