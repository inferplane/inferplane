...
// usageThenErrProvider yields ONE usage-bearing chunk, then breaks mid-stream.
// midStreamErrProvider's chunk carries no usage, so it cannot observe whether a
// settled partial stream was also metered.
type usageThenErrProvider struct{}

func (usageThenErrProvider) Name() string               { return "mock" }
...
func partialHandler(m *metrics.Metrics) *InvokeHandler {
	provs := map[string]providers.Provider{"p": usageThenErrProvider{}}
	models := map[string]config.ModelConfig{"claude-x": {Targets: []config.Target{{Provider: "p", Model: "up"}}}}
	holder := holderFor(provs, models)
	gov := governance.NewGovernor(nil, limiter.NewMemory(), budget.NewMemory(), nil)
	return NewInvokeHandlerMetrics(router.New(holder), holder, nil, gov, m, true)
}

func partialStreamReq() *http.Request {
	req := httptest.NewRequest("POST", "/model/claude-x/invoke-with-response-stream",
		strings.NewReader(`{"messages":[{"role":"user","content":"hi"}]}`))
	req.SetPathValue("modelId", "claude-x")
	return allowAll(req)
}

// TestInvokePartialStreamCountsTokensIntoMetrics — the partial branch settled
// and billed the delivered tokens but never called observeTokens, so
// gen_ai_client_token_usage_total under-reported every interrupted stream.
func TestInvokePartialStreamCountsTokensIntoMetrics(t *testing.T) {
	m := metrics.New()
	h := partialHandler(m)
...
func TestInvokePartialStreamMarksSpanPartialAndErrored(t *testing.T) {
